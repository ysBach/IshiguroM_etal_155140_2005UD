...
* ``Photometer.ipynb``, ``Photometer.html``, ``phot_log_all.txt``: ephemerides/PS1 query, centroid stars, Pillbox Aperture photometry, zeropoint fitting, target Circular Aperture photometry, standardization, and reduced magnitude calculation. All the logs are saved in (I did it in a rather manual copy-and-paste manner).
